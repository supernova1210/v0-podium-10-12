...
interface AtelierApprocheProps {
...
export default function AtelierApproche({
  accent,
  headingPre,
  headingHighlight,
  paragraphs,
  stats,
}: AtelierApprocheProps) {
      ...
      <div className="container mx-auto px-4">
        <Reveal>
          <div className="relative overflow-hidden bg-[#061952] rounded-[36px] px-7 md:px-16 py-14 md:py-18 max-w-7xl mx-auto">
            <div aria-hidden className={`absolute -top-12 -right-12 w-52 h-52 rounded-[40px] border-2 ${a.decoBorder}`} />
            <div aria-hidden className={`absolute top-10 right-16 w-24 h-24 rounded-[26px] hidden md:block ${a.decoSoft}`} />
            <div aria-hidden className="absolute -bottom-14 -right-10 w-44 h-44 rounded-[36px] bg-white/[0.04]" />

            <div className="relative max-w-3xl">
              <span className={`inline-flex items-center gap-2 font-poppins font-semibold text-xs tracking-[0.14em] uppercase ${a.eyebrowText}`}>
                <span className={`w-1.5 h-1.5 rounded-full ${a.dot}`} />
                L&apos;atelier en deux mots
              </span>

              <span aria-hidden className={`block font-poppins font-extrabold text-7xl leading-[0.4] h-10 mt-4 ${a.quote}`}>
                &ldquo;
              </span>

              <h2 className="font-poppins font-bold text-white leading-[1.15] tracking-tight text-2xl md:text-4xl mt-4 mb-6">
                {headingPre} <span className={a.highlight}>{headingHighlight}</span>
              </h2>
              ...
              <div className="flex flex-wrap gap-8 md:gap-12 mt-9 pt-8 border-t border-white/15">
                {stats.map((s, i) => (
                  <div key={i}>
                    <div className={`font-poppins font-bold text-3xl ${a.statNumber}`}>{s.n}</div>
